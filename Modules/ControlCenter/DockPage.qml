...
StyledFlickable {
    ...
    ColumnLayout {
        ...
        SettingsSection {
            ...
            Layout.fillWidth: true
            flat: true
            title: separatorsAnchor.title
            iconName: "space_bar"

            SettingsSearchAnchor {
                id: separatorsAnchor

                target: separatorsSection
                declaration:
                    '{"id":"general.dock.section.separators","route":"general.dock","title":"Separators","context":"DockPage","icon":"space_bar","aliases":["spacing","spacer"]}'
            }

            SettingsRow {
                Layout.fillWidth: true
                title: qsTr("Separator")
                supportingText: qsTr("Drag to the Dock or add at the end.")
                ...
                trailing: RowLayout {
                    ...
                    IconButton {
                        ...
                        tooltipText: qsTr("Drag to Dock")
                        Accessible.name: qsTr("Drag separator to Dock")
                        ...
                        Drag.onDragFinished: {
                            ...
                            Qt.callLater(() => {
                                separatorSource.capturedImage = null;
                        ...
                        DragHandler {
                            ...
                            onActiveChanged: {
                                if (active)
                                    separatorSource.grabToImage(result => {
                                        if (!separatorDrag.active)
                                            return;

                                        separatorSource.capturedImage = result;
                                        separatorSource.Drag.imageSource = result.url;
                                        separatorSource.Drag.active = true;
                                    });
                                else if (!separatorSource.nativeDragActive)
                                    separatorSource.Drag.active = false;
                            }
                        }

                        Component.onDestruction: {
                            if (nativeDragActive || capturedImage)
                                DockService.externalDragActive = false;
                        }
                    }
                    ...
                    ActionButton {
                        text: qsTr("Add")
                        iconName: "add"
                        onClicked: DockService.addSeparator()
                    }
                }
            }

            GeneralSliderSetting {
                title: qsTr("Separator spacing")
                from: 8
                to: 40
                stepSize: 2
                suffix: " px"
                value: DockService.separatorSize
                onMoved: value => {
                    return DockService.setOption("separatorSize", value);
                }
            }
        }
    }
